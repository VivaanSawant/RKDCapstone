...
###################### BONUS ##########################
    ...
    def curve_line_draw(self, startpos, curve_direction, line_start, num_steps):
        ...
        franka = Franka16384()
        print("\n\n 1 \n\n")
        R_fixed = robot.forward_kinematics(self.dh_parameters, franka.get_joints())[:3,:3]
        p_center = robot.forward_kinematics(self.dh_parameters, franka.get_joints())[:3,3]
        ...
        curve_fn = lambda s: np.block([
            [R_fixed, (np.array([ 
                                ...
                                [p_center[2] + radius*np.sin(curve_direction*np.pi*s)]
                                ])).reshape(3,1)],
            [0, 0, 0, 1]
        ])
        
        traj_curve = robot.draw_curve(franka.get_joints(), curve_fn, 100)
        robot.running_action(traj_curve)

        print("\n\n 2 \n\n")
        traj_curve_to_line = robot.compute_joint_trajectory(franka.get_joints(), line_start, num_steps)
        robot.running_action(traj_curve_to_line)
        #line
        dp = [0.0,-0.1,0.0]
        traj_line1 = robot.draw_line(franka.get_joints(), dp, num_steps)
        robot.running_action(traj_line1)

        return [traj_curve[-1], traj_line1]

        #######
    def flashlight(self, q_home, q_above_pick_place, q_lineTop, q_lineMiddle, q_lineBottom, flashlight_far, flashlight_middle, flashlight_near, q_curveStart, q_bin, num_steps):
        robot = Robot()
        franka = Franka16384()
        #open grippers
        ...
        robot.switch_flashlight(flashlight_near, q_above_pick_place, line_end2, curve_end2, q_bin, num_steps)

        curve_end3, line_end3 = robot.curve_line_draw(curve_end2, 1, q_lineBottom, num_steps)

        q_bin_traj = robot.compute_joint_trajectory(franka.get_joints(), q_bin, num_steps)
        robot.running_action(q_bin_traj)

        franka.open_gripper()
        
        traj_bin_home = robot.compute_joint_trajectory(franka.get_joints(), q_home, num_steps)
...
def main():
    ...
    q_above_flashlight = np.array([-6.43402714e-03,  1.93683010e-04, -2.71065634e-01, -2.48944621e+00,
 -7.12151012e-02,  2.57773746e+00,  8.42033321e-01]
)
    # 1. Choose a starting configuration
    q_curveStart = np.array([ 0.09981489, -0.35038494,  0.04496749, -2.45598581, -0.01842159,  3.60430649, 0.83886576])
    q_lineTop = np.array([ 0.10409193, -0.61418031,  0.13444654, -2.77560499, -0.01786074, 3.09400546, 0.94213729])
    q_lineMiddle = np.array([ 0.08718884, -0.23684348,  0.10876144, -2.77701321, -0.01541629,  3.64564966, 0.84030988])
    q_lineBottom = np.array([ 0.25492547, -0.55128371, -0.00715536, -2.82924959, -0.03123502,  3.50025698, 0.86129225])

    ###Change these 4 for demo
    flashlight_far = np.array([-0.02439576,  0.42565844, -0.21707064, -2.30422469,  0.03356562,  2.75428268, 0.81700025])
    flashlight_middle = np.array([-0.04109134,  0.50590948, -0.34178431, -2.22042008,  0.08068461,  2.72006962, 0.63589485])
    flashlight_near = np.array([-0.04659362,  0.57341467, -0.2576408,  -2.04453334,  0.17151329,  2.6836662, 0.76497263]
)

    q_bin = np.array([ 0.21079034,  0.30662803,  0.23867554, -2.12853207, -0.0188015, 2.44642589, 1.30182532])

    #MAIN:
    robot.flashlight(q_home, q_above_flashlight, q_lineTop, q_lineMiddle, q_lineBottom, flashlight_far,
    flashlight_middle, flashlight_near, q_curveStart, q_bin, num_steps)
